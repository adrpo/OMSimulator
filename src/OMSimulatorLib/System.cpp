...
oms_status_enu_t oms::System::exportToFMU(pugi::xml_node& modelVariables, pugi::xml_node& modelStructure, Snapshot& snapshot, int *valueReference) const
{
  // if we have top level connectors, use those
  if (connectors.size() > 1)
  {
    for(const auto& connector : connectors)
    {
      if (connector && (connector->isInput() || connector->isOutput()))
        if (oms_status_ok != connector->exportToFMU(modelVariables, valueReference))
          return logError("export of top level ssd connector in an FMU form failed");
    }
  }
  else // otherwise, dive into components and expose inputs/outputs
  {
    if (subelements.size() > 1)
    {
      // we ignore subsystems
      // dive into components
      for (const auto& component : components)
      {
        if (oms_status_ok != component.second->exportToFMU(modelVariables, snapshot, valueReference))
          return logError("export of component level ssd connector in an FMU form failed");
      }
    }
  }
